...
class StridedKeep(tf.keras.layers.Layer):
    """Keep only the last k elements in the time dimension."""
...
class StridedDrop(tf.keras.layers.Layer):
    """Drop the last k elements in the time dimension."""
    ...
    def compute_output_shape(self, input_shape):
        if self.k <= 0:
            return input_shape
        return (input_shape[0], input_shape[1] - self.k, input_shape[2], input_shape[3])


class Stream(tf.keras.layers.Layer):
    """Streaming layer for real-time inference."""
    
    def __init__(self, cell, ring_buffer_size_in_time_dim=0, use_one_step=False, **kwargs):
        super().__init__(**kwargs)
        self.cell = cell
    ...
    def call(self, inputs):
        # For training, just apply the cell directly
        if self.mode == "training":
            return self.cell(inputs)
        
        # For inference, implement streaming logic
        # This is simplified - in real implementation would use ring buffers
        return self.cell(inputs)
...
class MixConv:
    ...
    def __call__(self, inputs):
        ...
        for i, output in enumerate(x_outputs):
            ...
            x_outputs[i] = StridedDrop(features_drop)(output)
...
def model(flags, shape, batch_size):
    ...
    for filters, repeat, ksize, res in zip(
        ...
        for _ in range(repeat):
            ...
            if res:
                residual = StridedDrop(residual.shape[1] - net.shape[1])(
                    residual
                )
                net = net + residual
